...
const TableSampleClients = ({ columns }) => {
  const { clients } = useSampleClients('clinet')
  // console.log(clients)
    ...
    if (e.key == 'Enter' && e.target.value != '')
      clients.map((client, index) => {
        if (
          ...
          client.nickName == e.target.value ||
  ...
  const handelDeleteAction = async () => {
    ...
    await ApiAddData(`clinet/active/${id}`, { active: false }, (data) => {
                ...
                <tr key={client.id}>
                  ...
                  <td data-label="Created" className="lg:w-1 whitespace-nowrap">
                    <small className="text-gray-500 dark:text-slate-400">
                      <MomentP dateValue={client.created_at}></MomentP>
                    </small>
                  </td>
